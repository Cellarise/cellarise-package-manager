...
module.exports = function exec(logger) {
  ...
  var exports = {
    ...
    "execCommand": function execCommand(command, cwd, env, cb) {
      this._execCommand(command, cwd, env, true, cb);
    },
    "execCommandNoLogging": function execCommandNoLogging(command, cwd, env, cb) {
      this._execCommand(command, cwd, env, false, cb);
    },
    "_execCommand": function _execCommand(command, cwd, env, logging, cb) {
      cwd = cwd || process.cwd();
      env = env || process.env;
      if (logging) {
    ...
    "cloneFromGitNoLogging": function cloneFromGitNoLogging(repositoryPath, cloneToDir, cwd, cb) {
      this.execCommandNoLogging("cgitsh Auto-Git-clone " + repositoryPath + ".git " + cloneToDir, cwd, null,
        function cloneFromGitCallback(err, stdout) {
          err = null;
          cb(err, stdout);//do not pass through any error
        });
