...
const getJiraIssue = function (context, jiraOauthClient, callback) {
  ...
  const JIRA_ISSUE_KEY_ARR = R.defaultTo("", process.env.bamboo_repository_git_branch)
    .match(new RegExp(projectCode + "-\\d*", ""));

  if (R.isNil(JIRA_ISSUE_KEY_ARR) || JIRA_ISSUE_KEY_ARR.length === 0 || R.isNil(jiraOauthClient)) {
    callback(null, null);
    return;
  }

  jiraOauthClient.issue.getIssue({
    "issueKey": JIRA_ISSUE_KEY_ARR[0]
