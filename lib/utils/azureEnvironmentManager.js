...
const getDeploymentUrl = function (context) {
  const envName = jiraIssueManager.getJiraIssueKey(context);
  const azureConfig = require("./config")("azure")[process.env.bamboo_azure_config_code];

  if (R.isEmpty(envName) || R.isNil(envName)) {
    return azureConfig.defaults.webapp_deploy_url;
  }
  return getWebsiteSCMURLWithUser(azureConfig, envName);
};
...
function getDbPrimarySchema(context) {
  const azureConfig = require("./config")("azure")[process.env.bamboo_azure_config_code];
  const jiraIssueKey = jiraIssueManager.getJiraIssueKey(context);
  let dbSchemaName = jiraIssueKey.replace("-", "_").toUpperCase();
  if (R.isEmpty(jiraIssueKey) || R.isNil(jiraIssueKey)) {
    dbSchemaName = azureConfig.defaults.webapp_DbPrimarySchema;
  }
  return dbSchemaName;
}
...
function getDbBackupSchema(context) {
  const azureConfig = require("./config")("azure")[process.env.bamboo_azure_config_code];
  const jiraIssueKey = jiraIssueManager.getJiraIssueKey(context);
  let dbSchemaName = (jiraIssueKey + "_DAT").replace("-", "_").toUpperCase();
  if (R.isEmpty(jiraIssueKey) || R.isNil(jiraIssueKey)) {
    dbSchemaName = azureConfig.defaults.webapp_DbBackupSchema;
  }
  return dbSchemaName;
}
